...
describe Sequel::Database do
  ...
  describe "constraint violations" do
    ...
    cspecify "should raise Sequel::UniqueConstraintViolation when a unique constraint is violated", [:jdbc, :sqlite], [:db2], :vertica do
    ...
    cspecify "should raise Sequel::UniqueConstraintViolation when a unique constraint is violated for composite primary keys", [:jdbc, :sqlite], [:db2], :vertica do
    ...
    cspecify "should raise Sequel::CheckConstraintViolation when a check constraint is violated", :mysql, [:db2], [proc{|db| db.sqlite_version < 30802}, :sqlite], :vertica do
    ...
    cspecify "should raise Sequel::ForeignKeyConstraintViolation when a foreign key constraint is violated", [:jdbc, :sqlite], [:db2], :vertica do
      @db.create_table!(:test, :engine=>:InnoDB){primary_key :id; String :value}
