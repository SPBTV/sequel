...
describe "Bound Argument Types" do
  ...
  cspecify "should handle time type", [:do], [:jdbc, :sqlite], [:swift], [:oracle] do
    @ds.filter(:t=>:$x).prepare(:first, :ps_time).call(:x=>@vs[:t])[:t].to_datetime.must_equal @vs[:t].to_datetime
  end
  ...
  cspecify "should handle time type with fractional seconds", [:do, :postgres], [:jdbc, :sqlite], [:oracle], [:swift, :postgres] do
    ...
    @ds.filter(:t=>:$x).prepare(:first, :ps_time).call(:x=>fract_time)[:t].to_datetime.must_equal fract_time.to_datetime
  end
...
describe "Dataset#unbind" do
  ...
  cspecify "should handle dates and times", [:do], [:jdbc, :mssql], [:jdbc, :sqlite], [:swift], [:tinytds], :oracle do
    ...
    t = Time.now
    @ct[Time, t]
    @u[@ds.filter{c < t + 1}][:c].to_datetime.must_equal t.to_datetime
  end
